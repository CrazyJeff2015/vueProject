<template>
  <el-card class="box-card">
    ...
    <el-dialog title="用户上分记录" size="large" :visible.sync="showDetail" :show-close="false">
      ...
      <div style="margin-top: 20px;text-align: right"> 当月累计上分: <span style="color: rgb(3, 169, 244);"
...
<script>
  ...
  export default {
    ...
    methods: {
      showDetailFn(aid, uid ,user_score){
        this.detail.aid = aid;
        this.detail.uid = uid;
        this.userTotalScore = user_score;
        this.getData1();
      ...
      getData(val) {
        this.Data.pageApi = api.giveMoneyLog + '?page=' + (val ? val : this.Data.pageCurrent) + '&limit=' + this.Data.pagesize + this.parseParam(this.searchInfo);
        this.dataHandle();
      },
      getSize(val) {

        this.Data.pageApi = api.giveMoneyLog + '?page=' + this.Data.pageCurrent + '&limit=' + (val ? val : this.Data.pagesize) + this.parseParam(this.searchInfo);
        //搜索结束
        this.dataHandle();
      },
      dataHandle(){
      ...
      coutHandle(){
        var _this = this;
        this.$ajax.get(api.agentMouthCount)
          .then(function (response) {
