  ...
  <div>
    <el-card>
      <div slot="header" class="clearfix">
        <h4>消息管理 <el-button @click="showAdd=true" class="pull-right" size="small">新增消息</el-button></h4>
      </div>
      <el-table
        :data="Data.data"
        border
        ...
        <el-form :model="addData" ref="addData" :rules="addRule" label-width="150px">
          ...
          <el-form-item label="范围" required>
            <el-radio-group v-model="addData.type" @change="selectFn">
              ...
              <el-radio-button label="5">代运营</el-radio-button>
    ...
    <el-dialog :title="msgData1.title" :visible.sync="showMsg1" :show-close="false" size="large">
      <div style="margin-bottom:20px">生效时间：<span v-text="msgData1.start"></span></div>
      <div v-html="msgData1.content"></div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="showMsg1 = false">关 闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
//        console.log('editor change!', editor, html, text)
      ...
      getData(val) {
        this.Data.pageApi = api.AdminMes + '?type=7&page=' + (val ? val : this.Data.pageCurrent) + '&limit=' + this.Data.pagesize + this.parseParam(this.searchInfo);
        this.dataHandle();
      },
      getSize(val) {

        this.Data.pageApi = api.AdminMes + '?type=7&page=' + this.Data.pageCurrent + '&limit=' + (val ? val : this.Data.pagesize) + this.parseParam(this.searchInfo);
        //搜索结束
        this.dataHandle();
      },
      dataHandle(){
      ...
      getData1(val) {
        this.Data1.pageApi = api.AdminMes + '?type=7&is_operate=1&page=' + (val ? val : this.Data1.pageCurrent) + '&limit=' + this.Data1.pagesize;
        this.dataHandle1();
      },
      getSize1(val) {

        this.Data1.pageApi = api.AdminMes + '?type=7&is_operate=1&page=' + this.Data1.pageCurrent + '&limit=' + (val ? val : this.Data1.pagesize);
        //搜索结束
        this.dataHandle1();
      },
      dataHandle1(){
