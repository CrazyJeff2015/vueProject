<template>
  <el-card>
    ...
    <el-button :plain="true" type="info"  @click="showAdd = true" style="float: right">新增客服</el-button>
    ...
    <el-dialog title="修改客服" :visible.sync="showEdit" :show-close="false">
      <el-form :model="editData" ref="editData" label-width="150px">
        <el-form-item label="内容" prop="menu" required>
          <el-input type="textarea" autosize v-model="editData.body" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="新增客服" :visible.sync="showAdd" :show-close="false">
      <el-form :model="addData" ref="addData" label-width="150px">
        <el-form-item label="内容" prop="menu" required>
          <el-input type="textarea" autosize v-model="addData.body" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
<script>
  ...
  export default {
    ...
    methods: {
      switchChange(){
        this.switchValue == true ? (this.searchInfo.my = '1',this.searchInfo.other='') : (this.searchInfo.my = '',this.searchInfo.other='1');
        this.getData()
      },
      getData(val) {
        this.Data.pageApi = api.adminServiceList + '?page=' + (val ? val : this.Data.pageCurrent) + '&limit=' + this.Data.pagesize+this.parseParam(this.searchInfo) ;
        this.dataHandle();
      },
      getSize(val) {

        this.Data.pageApi = api.adminServiceList + '?page=' + this.Data.pageCurrent + '&limit=' + (val ? val : this.Data.pagesize)+this.parseParam(this.searchInfo) ;
        //搜索结束
        this.dataHandle();
      },
      dataHandle(){
      ...
      handleDelete(index, row) {
        this.delId = row.id;
        this.$confirm('此操作将删除该客户代码,是否继续?', '提示', {
