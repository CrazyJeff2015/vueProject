<template>
  <div>
    <el-card class="box-card">
      ...
      <el-form :inline="true" v-if="permissions.fast_juge">
      ...
      <el-form :inline="true" v-if="permissions.batch_juge">
        <el-form-item>
          <el-button type="primary" :plain="true"  @click="batch()">上周收益提交</el-button>
        </el-form-item>
        <el-form-item>
          <span style="color: red;font-size: 12px;">tips:该平台上周未提交收益将自动提交</span>
        </el-form-item>
      </el-form>
      <el-button v-if="permissions.update_profit" @click="fastJudge()">批量审批</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData(val) {
        this.Data.pageApi = api.adminMoneyList + '?page=' + (val ? val : this.Data.pageCurrent) + '&limit=' + this.Data.pagesize+this.parseParam(this.searchInfo);;
        this.dataHandle();
      },
      getSize(val) {
        this.Data.pageApi = api.adminMoneyList + '?page=' + this.Data.pageCurrent + '&limit=' + (val ? val : this.Data.pagesize)+this.parseParam(this.searchInfo);;
        //搜索结束
        this.dataHandle();
      },
      dataHandle(){
