  ...
  <div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template scope="scope">
            <el-button icon="edit" size="small" @click="editFn(scope.row.id,scope.row.tax_speed,scope.row.user_ai_speed)"></el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="设置" :visible.sync="showSet" :show-close="false">
      <el-form :model="setData" ref="setData" :rules="rules" label-width="150px">
        <el-form-item label="数量:" prop="cur_tax_value" required>
          <el-input v-model="setData.cur_tax_value" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="速度A:" prop="tax_speed" required>
          <el-input v-model="setData.tax_speed" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="速度B:" prop="user_ai_speed" required>
          <el-input v-model="setData.user_ai_speed" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="方向:" required>
          <el-radio-group v-model="setData.type">
            <el-radio :label="1">收</el-radio>
            <el-radio :label="-1">放</el-radio>
          </el-radio-group>
      </el-form-item>
      </el-form>
  ...
  export default {
    ...
    data: function () {
      ...
      var checkSpeed1 = (rule, value, callback) => {
        ...
        if (reg.test(value) == false) {
          callback(new Error('必须包含2位小数，范围(0.01 ~ 0.99)'));
        } else {
          callback();
        }
      }
    ...
    methods: {
      ...
      createStateFilter(queryString) {
        ...
        editFn(id,value1,value2){
          this.editId = id;
          this.setData.tax_speed = value1.toString();
          this.setData.user_ai_speed = value2.toString();
          this.showSet=true
      ...
      getData: function () {
        var _this = this;
        this.$ajax({
          url: api.getFishList+'?'+_this.parseParam(_this.searchInfo),
          method: 'get'
        }).then(function (response) {
      ...
      sentFn: function () {
        var _this = this;
        this.$ajax({
          url: api.updateFishInfo + _this.editId,
          method: "post",
          data: _this.setData
        }).then(function (response) {
