<template>
  <el-card class="am">
    ...
    <h4>问答管理 <el-button @click="showAdd=true" class="pull-right" size="small">新增问答</el-button></h4>
      <i>每日只可创建一条问答</i>
    </div>
    <el-table
      :data="Data.data"
    ...
    <el-dialog title="新增消息" size="large" :visible.sync="showAdd" :show-close="false">
      <el-form :model="addData" ref="addData" :rules="addRule" label-width="150px">
        ...
        <el-form-item label="内容:" prop="content" required>
          <el-input  type="textarea" autosize  :autosize="{ minRows: 4, maxRows: 20}" placeholder="请输入内容" v-model="addData.content" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      reFn(){
        ...
        var content = this.msgData.content +'<div class="ag">'+ this.loginAgentName + ':' + this.reContent + '<br>'+time+'</div><br><br>';
        var _this = this;
        this.$ajax({
      ...
      addMessage: function () {
        ...
        var content = '<div class="ag">'+this.loginAgentName + ':' + this.addData.content + '<br>'+this.addData.start+'</div><br><br>';
      ...
      getData(val) {
        this.Data.pageApi = api.agentMQ + '?type=4&page=' + (val ? val : this.Data.pageCurrent) + '&limit=' + this.Data.pagesize + this.parseParam(this.searchInfo);
        this.dataHandle();
      },
      getSize(val) {

        this.Data.pageApi = api.agentMQ + '?type=4&page=' + this.Data.pageCurrent + '&limit=' + (val ? val : this.Data.pagesize) + this.parseParam(this.searchInfo);
        //搜索结束
        this.dataHandle();
      },
      dataHandle(){
